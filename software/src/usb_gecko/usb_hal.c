...
#if defined(_SILICON_LABS_GECKO_INTERNAL_SDID_100) || defined(_SILICON_LABS_GECKO_INTERNAL_SDID_106)
...
USB_Status_TypeDef USBDHAL_CoreInit(uint32_t totalRxFifoSize, uint32_t totalTxFifoSize)
{
  uint8_t i, j;
  ...
  //!test
  GPIO->P[5].MODEL = (GPIO->P[5].MODEL & ~_GPIO_P_MODEL_MODE5_MASK) | GPIO_P_MODEL_MODE5_PUSHPULL;  /* Enable VBUSEN pin (PF5) */
  ...
  for(i = 1; i <= NUM_EP_USED; i++)
  {
    ...
    if(ep->state != D_EP_IDLE)
    {
      ...
      if(ep->xferCompleteCb)
      {
        ...
        if((dev->lastState == USBD_STATE_CONFIGURED) && (dev->state     == USBD_STATE_ADDRESSED))
        {
