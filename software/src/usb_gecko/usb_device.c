...
int USBD_Read(int epAddr, void *data, int byteCount, USB_XferCompleteCb_TypeDef callback)
{
  ...
  if((byteCount > MAX_XFER_LEN) || ( (byteCount / ep->packetSize) > MAX_PACKETS_PR_XFER))
  {
...
int USBD_RemoteWakeup(void)
{
  ...
  if((dev->state != USBD_STATE_SUSPENDED) || (dev->remoteWakeupEnabled == false))
  {
    ATOMIC_EXIT();
...
void USBD_SetUsbState(USBD_State_TypeDef newState)
{
  ...
  if((dev->callbacks->usbStateChange) && (currentState != newState))
  {
