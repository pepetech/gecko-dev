...
void crypto_sha256(uint8_t *pubData, uint32_t ulDataSize, uint8_t pubDigest[32])
{
    ...
    CRYPTO->WAC = (CRYPTO->WAC & ~_CRYPTO_WAC_RESULTWIDTH_MASK) | CRYPTO_WAC_RESULTWIDTH_256BIT;

    for(uint8_t i = 0; i < 8; i++)
        CRYPTO->DDATA1 = pulSHABlock[i];

    CRYPTO->SEQ0 = CRYPTO_CMD_INSTR_DDATA1TODDATA0 | (CRYPTO_CMD_INSTR_SELDDATA0DDATA1 << 8) | (CRYPTO_CMD_INSTR_EXEC << 16);
    ...
    while(ulByteSize >= 64)
    {
        for(uint8_t i = 0; i < 16; i++)
            CRYPTO->QDATA1BIG = ((uint32_t *)pubData)[i];

        CRYPTO->SEQ0 = CRYPTO_CMD_INSTR_SHA | (CRYPTO_CMD_INSTR_MADD32 << 8) | (CRYPTO_CMD_INSTR_DDATA0TODDATA1 << 16) | (CRYPTO_CMD_INSTR_EXEC << 24);
    ...
    if(ulBlockSize > 56)
    {
        ...
        for(uint8_t i = 0; i < 16; i++)
            CRYPTO->QDATA1BIG = pulSHABlock[i];
    ...
    *(uint32_t *)&pubSHABlock[60] = __REV((uint32_t)ullBitSize & 0xFFFFFFFF);
    ...
    for(uint8_t i = 0; i < 8; i++)
        ((uint32_t *)pubDigest)[i] = CRYPTO->DDATA0BIG;
}
